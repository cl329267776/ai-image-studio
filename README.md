...
上传商品实拍原图 → 调用即梦(火山引擎)云端 API → 生成 5 张 800×800 主图 + 2 张九宫格详情图(宽 800、高 ≤1200)→ 按日期目录保存 → 浏览器预览下载。
...
- 自动生成 **5 张主图**(800×800):
  ...
  2. 商务场景图(浅灰台面/自然光)
  3. 材质细节图(深色背景突出纹理与卷边工艺)
  4. 定制展示图(突出可印刷罐身)
  5. 45° 展示图(完整形态)
- 自动生成 **2 张详情图**(3×3 九宫格,宽 800、高 ≤1200):由主图+原图拼格,两种排列顺序
- 支持「自定义提示词」:填入自己的风格描述,替代默认 4 种风格
- 重新生成:页面刷新后可再次上传重跑
